...
class TwitterOauth
	...
	def dump_followers( screen_name = nil )
		...
		# Twitter REST API Method: direct_messages  by prasad
	def home_timeline( since_id = nil, max_id = nil , count = nil, page = nil )
		params = (
			{ :since_id => since_id, :max_id => max_id, :count => count, :page => page }.collect { |n| "#{n[0]}=#{n[1]}" if n[1] }
		).compact.join('&')
		response = access_token.get('/statuses/home_timeline.json' + ( params.empty? ? '' : '?' + params ) )
		case response
		when Net::HTTPSuccess
			messages=JSON.parse(response.body)
			raise TwitterOauth::UnexpectedResponse unless messages.is_a? Array
		...
		puts "Exception in timelines: #{err}"
		raise err
	end
