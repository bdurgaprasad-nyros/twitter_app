...
module OauthSystem
	...
	def callback
		...
		## prasad extra methods 
		#puts user_info.inspect
		#~ flash[:notice]=user_info["followers_count"]
		#~ @user_info = user_info
		...
		if @member
			...
			@member = Member.new({ 
				...
				:friends_count => user_info['friends_count'].to_i ,
				:followers_count => user_info['followers_count'].to_i ,
				:statuses_count => user_info['statuses_count'].to_i })
		end
		if @member.save!
...
protected
	...
	def direct_messages( since_id = nil, max_id = nil , count = nil, page = nil )
		...
		return
       	...
       	#~ # Twitter REST API Method: direct_messages
	def home_timeline( since_id = nil, max_id = nil , count = nil, page = nil )
		self.twitagent.home_timeline( since_id, max_id, count, page )
	rescue => err
		RAILS_DEFAULT_LOGGER.error "Failed to get time lines via OAuth for #{current_user.inspect}"
		flash[:error] = "Twitter API failure (getting time lines)"
		return
	end
